...
impl HeightMap {
    ...
    pub fn placeholders(&self) -> FxHashSet<Point3<i32>> {
        Self::chunk_area_points(self.0.keys().copied())
            ...
            .into_iter()
            .flat_map(|coords| {
                let bottom = self.bottom(coords);
                let top = self.top(coords).unwrap_or_else(|| unreachable!());
    ...
    fn bottom(&self, coords: Point2<i32>) -> i32 {
        self.0
            .get(&coords)
            .map(|&height| height + 1)
            .unwrap_or(World::Y_RANGE.start - 1)
    }

    fn top(&self, coords: Point2<i32>) -> Option<i32> {
